...
$app->group('/jobs', function () use ($app) {
    ...
    $app->get('/new', 'isJobPostAllowed', 'isBanned', function () use ($app) {
        ...
        $app->render(THEME_PATH . 'job.new.php', array('lang' => $lang,
            'seo_url' => $seo_url,
            'seo_title' => $seo_title,
            'seo_desc' => $seo_desc,
            'token' => $token,
    ...
    $app->get('/:id/publish/:token', 'isJobPostAllowed', 'isBanned', 'isValidReferrer', function ($id, $token) use ($app) {
        ...
        if (isset($job) && $job->id) {
            ...
            $app->render(THEME_PATH . 'job.publish.php', array('lang' => $lang,
                'seo_url' => $seo_url,
                'seo_title' => $seo_title,
                'seo_desc' => $seo_desc,
                'job' => $job,
    ...
    $app->get('/:id/edit/:token', 'isJobPostAllowed', 'isBanned', function ($id, $token) use ($app) {
        ...
        if (isset($job) && $job) {
            ...
            $app->render(THEME_PATH . 'job.review.php', array('lang' => $lang,
                'seo_url' => $seo_url,
                'seo_title' => $seo_title,
                'seo_desc' => $seo_desc,
                'job' => $job,
    ...
    $app->get('/:id(/:title)', function ($id, $title = null) use ($app) {
        ...
        if (isset($job) && $job->id && $job->status == ACTIVE) {
            ...
            $app->render(THEME_PATH . 'job.show.php', array('lang' => $lang,
                'seo_url' => $seo_url,
                'seo_title' => $seo_title,
                'seo_desc' => $seo_desc,
                'job' => $job,
