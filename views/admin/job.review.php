                ...
                <div class="col-sm-6">
                    <select name="category" id="category" class="form-control input-lg">
                        <?php foreach ($categories as $category): ?>
                            <option value="<?php _e($category->id); ?>" <?php
                            if ($category->id == $job->category) {
                                _e("selected");
                            }
                            ?>><?php _e($category->name); ?></option>
<?php endforeach; ?>
                    </select>
                </div>
            </div>
                ...
                <div class="col-sm-6">
                    <select name="city" id="city" class="form-control input-lg">
                        <?php foreach ($cities as $city): ?>
                            <option value="<?php _e($city->id); ?>" <?php
                                    if ($city->id == $job->city) {
                                        _e("selected");
                                    }
                                    ?>><?php _e($city->name); ?></option>
<?php endforeach; ?>
                    </select>
                </div>
            </div>
...
<?php endif; ?>
            ...
            <div class="form-group">
                ...
                <div class="col-sm-8">
                    ...
                    <p class="lead"><input type="checkbox" id="is_featured" name="is_featured" <?php
if ($job->is_featured) {
    echo 'checked';
}
?> /> <?php echo $lang->t('jobs|feature_yes'); ?></p>
                </div>
            </div>
