    ...
    <div class="col-md-3">
        <h4><?php
            if ($category && $category->name) {
                echo $lang->t('admin|btn_edit') . "<span class='pull-right'><a href='" . ADMIN_MANAGE . '/categories' . "'>" . $lang->t('admin|btn_add_new') . "</a></span>";
            } else {
                echo $lang->t('admin|btn_add_new');
            }
            ?></span></h4>
        <form class="form" role="form" method="post" action="<?php _e(ADMIN_MANAGE . '/categories'); ?>">
            <div class="form-group">
                <input type="text" class="form-control" name="name" placeholder="<?php echo $lang->t('admin|tbl_name'); ?>" value="<?php
                       if ($category && $category->name) {
                           _e($category->name);
                       }
            ?>" required />
            ...
            <div class="form-group">
                <input type="text" class="form-control" name="description" placeholder="<?php echo $lang->t('admin|tbl_meta_desc'); ?>" maxlength="150" value="<?php
                       if ($category && $category->description) {
                           _e($category->description);
                       }
            ?>" required />
            </div>
            <div class="form-group">
                <input type="text" class="form-control" name="url" placeholder="<?php echo $lang->t('admin|slug_url'); ?>" value="<?php
            ...
            <div class="form-group">
                <input type="text" class="form-control" name="sort" placeholder="<?php echo $lang->t('admin|tbl_sort'); ?>" value="<?php
