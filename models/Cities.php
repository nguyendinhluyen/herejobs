...
class Cities {
    ...
    public function findCityJobs($start, $limit) {
        $jobs = R::findAll('jobs', " status=1 AND city=:city ORDER BY created DESC LIMIT :start, :limit ", array(':city' => $this->_id, ':start' => $start, ':limit' => $limit));
        return $jobs;
    }

    public function findAllCityJobs() {
        $jobs = R::findAll('jobs', " status=1 AND city=:city ORDER BY created DESC LIMIT 0, 100", array(':city' => $this->_id));
        return $jobs;
    }
