...
class Categories {
    ...
    public function findCategoryJobs($start, $limit) {
        $jobs = R::findAll('jobs', " status=1 AND category=:category ORDER BY created DESC LIMIT :start, :limit ", array(':category' => $this->_id, ':start' => $start, ':limit' => $limit));
        return $jobs;
    }

    public function findAllCategoryJobs() {
        $jobs = R::findAll('jobs', " status=1 AND category=:category ORDER BY created DESC LIMIT 0, 100", array(':category' => $this->_id));
        return $jobs;
    }
